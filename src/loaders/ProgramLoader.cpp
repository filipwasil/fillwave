...
const std::string vsDebugger = gGLVersion + gGLVertexPrecision + gGLVSAttributesQuad +
...
const std::string vsSkybox = gGLVersion + gGLVertexPrecision + gGLVSAttributesPosition +
   "uniform mat4 uModelMatrix;\n"
...
const std::string fsQuad = gGLVersion + gGLVertexPrecision +
   "in vec2 vPosition;\n"
   "out vec4 fColour;\n"
...
const std::string fsCursor = gGLVersion + gGLFragmentPrecision +
...
const std::string vsCursor = gGLVersion + gGLVertexPrecision +
...
const std::string vsCube = gGLVersion +
   "uniform mat4 uMVP;\n"
   "void main() {\n"
   "   vec4 vertexPosition;\n"
   "   switch(gl_VertexID) {\n"
...
const std::string fsAOGeometry = gGLVersion +
   "in vec3 vMVPosition;\n"
...
const std::string fsDR = gGLVersion +
...
std::string vsDRShaderQuad = gGLVersion +
   "void main() {\n"
   "   vec4 vertexPosition;\n"
   "   switch(gl_VertexID) {\n"
   "      case 0:\n"
...
std::string fsDRLightDirectional = gGLVersion + gGLLightDefinitions +
...
std::string vsDRLightPoint =
      gGLVersion + gGLVSAttributes + vsSimpleVertexPass;

std::string fsDRLightPoint = gGLVersion + gGLLightDefinitions +
...
std::string fsDRLightSpot = gGLVersion + gGLLightDefinitions +
...
const std::string fsAOColor = gGLVersion +
   "in vec2 vTextureCoordinate;\n"
...
const std::string fsStartup = gGLVersion +
   "#define MAX_ITER 3\n"
...
pProgram ProgramLoader::getShadow(Engine* engine) {
   return engine->storeProgram("shadow_mapping",
             ...
             engine->storeShaderVertex("fillwave_internal_shadowing.vert", vsShadow));
}

pProgram ProgramLoader::getShadowColorCoded(Engine* engine) {
   return engine->storeProgram("shadow_mapping_color",
             engine->storeShaderFragment("fillwave_internal_shadowing_color.frag", fsShadowColorCoded) +
             engine->storeShaderVertex("fillwave_internal_shadowing_color.vert", vsShadowColorCoded));
}

pProgram ProgramLoader::getShadowWithAnimation(Engine* engine) {
   return engine->storeProgram("shadow_mapping_animated",
             engine->storeShaderFragment("fillwave_empty.frag", fsEmpty) +
             engine->storeShaderVertex("fillwave_internal_shadowing.vert", vsShadowAnimated));
}
...
pProgram ProgramLoader::getShadowColorCodedWithAnimation(Engine* engine) {
   return engine->storeProgram("shadow_mapping_animated_color",
             engine->storeShaderFragment("fillwave_internal_shadowing.frag", fsShadowColorCoded) +
             engine->storeShaderVertex("fillwave_internal_shadowing.vert", vsShadowColorCodedAnimated));
}

pProgram ProgramLoader::getDebugger(Engine* engine) {
   return engine->storeProgram("debugger",
             engine->storeShaderFragment("fillwave_ifillwave__debugger.frag", fsDebugger) +
             engine->storeShaderVertex("fillwave_internal_debugger.vert", vsDebugger));
}
...
pProgram ProgramLoader::getSkybox(Engine* engine) {
   return engine->storeProgram("internal_skybox",
             ...
             engine->storeShaderVertex("fillwave_internal_skybox.vert", vsSkybox));
}

pProgram ProgramLoader::getSkyboxDR(Engine* engine) {
   return engine->storeProgram("internal_skybox_dr",
             engine->storeShaderFragment("fillwave_internal_skybox_dr.frag", fsSkyboxDR) +
             engine->storeShaderVertex("fillwave_internal_skybox.vert", vsSkybox));
}
...
pProgram ProgramLoader::getText(Engine* engine) {
   return engine->storeProgram("text",
             ...
             engine->storeShaderVertex("fillwave_internal_text.vert", vsText));
}

pProgram ProgramLoader::getTextBold(Engine* engine) {
   return engine->storeProgram("text_bold",
             engine->storeShaderFragment("fillwave_internal_text_bold.frag", fsTextBold) +
             engine->storeShaderVertex("fillwave_internal_text.vert", vsText));
}
...
pProgram ProgramLoader::getParticleGPUEmiter(Engine* engine) {
   return engine->storeProgram("particles_gpu_emiter",
             engine->storeShaderFragment("fillwave_empty.frag", fsEmpty) +
             engine->storeShaderVertex("fillwave_particles_gpu_emiter.vert", vsGPUEmiter),
             GL_TRUE);
}

pProgram ProgramLoader::getParticleGPU(Engine* engine) {
   return engine->storeProgram("particles_gpu_program",
             engine->storeShaderFragment("fillwave_particles_gpu.frag", fsParticlesGPU) +
             engine->storeShaderVertex("fillwave_particles_gpu.vert", vsParticlesGPU));
}

pProgram ProgramLoader::getParticleCPU(Engine* engine) {
   return engine->storeProgram("particles_cpu_program",
             engine->storeShaderFragment("fillwave_particles_cpu.frag", fsParticlesCPU) +
             engine->storeShaderVertex("fillwave_particles_cpu.vert", vsParticlesCPU));
}
...
pProgram ProgramLoader::getQuadCustomFragmentShader(Engine* engine, const std::string& shaderPath) {
...
pProgram ProgramLoader::getAmbientOcclusionGeometry(Engine* engine) {
   return engine->storeProgram("ambient_occlusion_geometry",
             engine->storeShaderFragment("fillwave_ambient_occlusion_geometry.frag", fsAOGeometry) +
             engine->storeShaderVertex("fillwave_ambient_occlusion_geometry.vert", vsAOGeometry));
}
...
pProgram ProgramLoader::getDRPointLights(Engine* engine) {
   ...
   engine->storeShaderVertex("fillwave_ds_point.vert", vsDRLightPoint));
}

pProgram ProgramLoader::getAmbientOcclusionColor(Engine* engine) {
   pProgram p = engine->storeProgram("ambient_occlusion_color",
             engine->storeShaderFragment("fillwave_ambient_occlusion_color.frag", fsAOColor) +
             engine->storeShaderVertex("fillwave_ambient_occlusion_color.vert", vsAOColor));
...
inline pProgram ProgramLoader::getDefaultDR(Engine* engine) {
   ...
   return engine->storeProgram("dr_g",
         ...
         engine->storeShaderVertex("fillwave_default.vert", loaderVertex.getSource()));
}
...
inline pProgram ProgramLoader::getDefaultFR(Engine* engine) {
   ...
   return engine->storeProgram("default",
          engine->storeShaderFragment("fillwave_default.frag", loaderFragment.getSource()) +
          engine->storeShaderVertex("fillwave_default.vert", loaderVertex.getSource()));
}
...
pProgram ProgramLoader::getDefaultBonesDR(Engine* engine) {
   ...
   return engine->storeProgram("default_animated",
         ...
         engine->storeShaderVertex("fillwave_default_animated.vert", loaderVertex.getSource()));
}
...
void ProgramLoader::initDefaultUniforms(core::Program* program) {
   ...
   if (location[0] != -1) core::Uniform::push(location[0], glm::vec4(0.2, 0.2, 0.2, 1.0));
   if (location[1] != -1) core::Uniform::push(location[1], 0);
   if (location[2] != -1) core::Uniform::push(location[2], 0);
   if (location[3] != -1) core::Uniform::push(location[3], false);
   if (location[4] != -1) core::Uniform::push(location[4], false);
   if (location[5] != -1) core::Uniform::push(location[5], false);
   if (location[6] != -1) core::Uniform::push(location[6], false);
   if (location[7] != -1) core::Uniform::push(location[7], FILLWAVE_DIFFUSE_UNIT);
   if (location[8] != -1) core::Uniform::push(location[8], FILLWAVE_NORMAL_UNIT);
   if (location[9] != -1) core::Uniform::push(location[9], FILLWAVE_SPECULAR_UNIT);
