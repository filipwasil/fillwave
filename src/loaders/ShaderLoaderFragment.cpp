...
ShaderLoaderFragment::ShaderLoaderFragment(eShaderLoaderOpenGLVersion version,
...
const std::string ShaderLoaderFragment::getSource() {
...
//   end += "fColor" + strings::to_string(0) + " = color* 0.001 + texture(uPointShadowMap0, toLightDirection).xyzr;\n";
   ...
   return version + inputs + outputs + lightsMaximum + lightStructDefinition + lightUniforms + lightSamplerUniforms + effectsUniforms + textures + functions + main + end;
}
