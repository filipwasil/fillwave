...
Program::Program(const std::vector<pShader>& shaders,
...
void Program::attach(pShader shader) {
   if(mHandle) {
      ...
      FLOG_CHECK("attach shader %d to program %d", shader->getHandle(), mHandle);
   }
}
...
void Program::uniformPush(string name, GLfloat data) {
   ...
   FLOG_ERROR("Can not find \"float %s\" uniform name in program", name.c_str());
}
...
void Program::uniformPush(string name, glm::vec2 data) {
   ...
   FLOG_ERROR("Can not find \"glm::vec2 %s\" uniform name in program", name.c_str());
}
...
void Program::uniformPush(string name, glm::vec3 data) {
   ...
   FLOG_ERROR("Can not find \"glm::vec3 %s\" uniform name in program", name.c_str());
}
...
void Program::uniformPush(string name, glm::vec3* data, GLuint size) {
   ...
   FLOG_ERROR("Can not find %d \"vec3 %s\" uniforms name in program", size, name.c_str());
}
...
void Program::uniformPush(string name, glm::vec4 data) {
   ...
   FLOG_ERROR("Can not find \"glm::vec4 %s\" uniform name in program", name.c_str());
}
...
inline void Program::getUniforms() {
   if (mUniforms.empty()) {
      ...
      for(GLint i=0; i<howMany; ++i) {
          ...
          FLOG_DEBUG("Uniform name=%s, location=%d, type=%d, num=%d",
                name,
                location,
                type,
                num);
...
void Program::getUniformBlock(std::string name, GLuint bindingPoint) {
   ...
   for(GLint i=0; i<howMany; ++i) {
      ...
      glGetActiveUniformBlockName( mHandle, GLuint(i), sizeof(nameFromProgram)-1, &name_len, nameFromProgram );
      ...
      if (std::string(nameFromProgram) == name) {
         FLOG_DEBUG("Uniform block name=%s, index=%d, size=%d ", std::string(name).c_str(), GLuint(i), uniformBlockSize);
         GLuint blockIndex = glGetUniformBlockIndex(mHandle, nameFromProgram);
         glUniformBlockBinding(mHandle, blockIndex, bindingPoint);
         mUnifromBuffers.push_back(puUniformBuffer(new UniformBuffer(std::string(name), blockIndex, uniformBlockSize, bindingPoint, GL_DYNAMIC_DRAW)));
      }
   }
}
...
void Program::reload() {
   ...
   for (auto it : mShaders) {
      attach(it);
      FLOG_CHECK("Failed to attach shader of type %s", it->getTypeString().c_str());
   }
   if (!mDelayedLinking) {
      link();
   } else {
