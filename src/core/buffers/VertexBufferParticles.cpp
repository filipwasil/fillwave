...
VertexBufferParticles::VertexBufferParticles(std::vector<GLfloat>& velocities,
...
void VertexBufferParticles::log() {
   for (auto it : mDataVertices) {
         FLOG_ERROR("Vertex written: %f %f %f %f", it.velocity[0],it.velocity[1],it.velocity[2],it.startTime);
   }
}
