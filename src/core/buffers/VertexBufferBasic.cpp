...
VertexBufferBasic::VertexBufferBasic(const fMesh* shape,
      ...
      for (GLuint i=0; i<shape->mNumBones; i++) {
         for (GLuint j=0; j<shape->mBones[i]->mNumWeights; j++) {
            ...
            if (boneIdForEachVertex[VertexID] < FILLWAVE_MAX_BONES_DEPENDENCIES) {
               mDataVertices[VertexID].mBoneID[boneIdForEachVertex[VertexID]] = boneManager->getId(shape->mBones[i]->mName.C_Str());
               mDataVertices[VertexID].mBoneWeight[boneIdForEachVertex[VertexID]] = Weight;
               boneIdForEachVertex[VertexID]++;
            } else {
               FLOG_FATAL("Crater can handle maximum %d bone dependencies.", FILLWAVE_MAX_BONES_DEPENDENCIES);
            }
         }
      }
   }
...
VertexBufferBasic::VertexBufferBasic(terrain::TerrainConstructor* constructor,
                                     GLint chunkDensity,
                                     GLfloat gapSize,
...
:VertexBuffer<VertexBasic>(dataStoreModification) {
   ...
   for (float z=0; z<=chunkDensity; z++) {
      for (float x=0; x<=chunkDensity; x++) {
            ...
            vertex.mPosition[1] = constructor->calculateHeight(vertex.mTextureUV[0], vertex.mTextureUV[1]); // calculate height 0.0f;
   ...
   for(int i = 0; i < indices.size() ; i += 3) {
      ...
      glm::vec3 v0(mDataVertices[indices[i]].mPosition[0], mDataVertices[indices[i]].mPosition[1], mDataVertices[indices[i]].mPosition[2]);
      glm::vec3 v1(mDataVertices[indices[j]].mPosition[0], mDataVertices[indices[j]].mPosition[1], mDataVertices[indices[j]].mPosition[2]);
      glm::vec3 v2(mDataVertices[indices[z]].mPosition[0], mDataVertices[indices[z]].mPosition[1], mDataVertices[indices[z]].mPosition[2]);
...
VertexBufferBasic::VertexBufferBasic(std::vector<core::VertexBasic>& vertices, GLuint dataStoreModification)
...
glm::vec3 VertexBufferBasic::getOcclusionBoxSize() {
   ...
   for ( GLuint i=0; i<mTotalElements; i++) {
      if (mDataVertices[i].mPosition[0] > maximum.x) maximum.x = mDataVertices[i].mPosition[0];
      if (mDataVertices[i].mPosition[1] > maximum.y) maximum.y = mDataVertices[i].mPosition[1];
      if (mDataVertices[i].mPosition[2] > maximum.z) maximum.z = mDataVertices[i].mPosition[2];
      if (mDataVertices[i].mPosition[0] < minimum.x) minimum.x = mDataVertices[i].mPosition[0];
      if (mDataVertices[i].mPosition[1] < minimum.y) minimum.y = mDataVertices[i].mPosition[1];
      if (mDataVertices[i].mPosition[2] < minimum.z) minimum.z = mDataVertices[i].mPosition[2];
   }
...
void VertexBufferBasic::log() {
   for (auto it : mDataVertices) {
      ...
      FLOG_INFO("Vertex normal: %f %f %f", it.mNormal[0], it.mNormal[1], it.mNormal[2]);
      FLOG_INFO("Vertex position: %f %f %f", it.mPosition[0], it.mPosition[1], it.mPosition[2]);
   }
}
