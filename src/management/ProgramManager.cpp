...
pProgram ProgramManager::add(const std::string& name, const std::vector<pShader>& shaders, GLboolean skipLinking) {
...
pProgram ProgramManager::get(const std::string& name) {
   ...
   FLOG_DEBUG("Program %s not found in manager. Returning empty pointer",name.c_str());
   return pProgram();
}
