...
pLightSpot Engine::storeLightSpot(
	...
	return mLightManager->addLightSpot(
			mTextureManager->getShadow2D(mWindowWidth, mWindowHeight), position,
...
pLightPoint Engine::storeLightPoint(
	...
	return mLightManager->addLightPoint(
			mTextureManager->getShadow3D(mWindowWidth, mWindowHeight), position,
			color, entity);
}
...
inline pShader Engine::storeShader(
...
pTexture Engine::storeTexture(
	...
	return mTextureManager->get(texturePath, mapType, compression);
}

pTexture2DRenderableDynamic Engine::storeTextureDynamic(
		const std::string& fragmentShaderPath) {
	const std::string path = fragmentShaderPath;
	pProgram program = mProgramLoader.getQuadCustomFragmentShader(this,
			fragmentShaderPath);
	pTexture2DRenderableDynamic t = mTextureManager->getDynamic(path, program,
...
pTexture3D Engine::storeTexture3D(
	...
	return mTextureManager->get(posX, negX, posY, negY, posZ, negZ);
}
...
void Engine::unregisterCallback(
		std::vector<actions::EngineCallback*>& callbacks,
...
pText Engine::storeText(
	...
	pTexture t = mTextureManager->get(fontName + ".png",
...
puPhysicsMeshBuffer Engine::getPhysicalMeshBuffer(
	...
	const fScene* scene = mImporter.ReadFile(
			(getExecutablePath() + shapePath).c_str(),
...
void Engine::addPostProcess(
	...
	pProgram program = mProgramLoader.getQuadCustomFragmentShader(this,
			fragmentShaderPath);
	common::PostProcessingPass pass(program, mWindowWidth, mWindowHeight,
			mTextureManager->getDynamic(fragmentShaderPath, program,
					glm::ivec2(mWindowWidth, mWindowHeight)), lifeTime);
...
void Engine::configureFPSCounter(
		...
		GLfloat size) {
	if (fontName.size() > 1) {
		mFPSText = storeText("", fontName, xPosition, yPosition, size);
...
inline void Engine::initDeferredShading() {
	...
	mDeferredPointLight = puMesh(
			new models::Mesh(this, material, buildTextureRegion(pTexture()),
