...
void perform() {
	...
	for (GLint i = 0; i < SPHERES; i++) {
		...
		sphere->attachHierarchyCallback(make_unique<LoopCallback>(std::move(make_unique_container<SequenceCallback>(
		                                   ...
		                                   make_unique<TimedRotateCallback>(sphere.get(), downY, glm::radians(90.0f), 2.0f + i * 0.5f, BounceEaseOut),
		                                   make_unique<TimedMoveCallback>(sphere.get(), upY, 2.0f + i * 0.5f),
		                                   make_unique<TimedMoveCallback>(sphere.get(), downY, 2.0f + i * 0.5f))),
		                                FILLWAVE_ENDLESS));
...
void showDescription() {
	pText hint0 = ContextGLFW::mGraphicsEngine->storeText("Fillwave example callbacks", "fonts/Titania", glm::vec2(-0.95, 0.80), 100.0);
	pText hint3 = ContextGLFW::mGraphicsEngine->storeText("Use 'S' for camera back", "fonts/Titania", glm::vec2(-0.95, -0.50), 70.0);
	pText hint4 = ContextGLFW::mGraphicsEngine->storeText("Use 'W' for camera forward", "fonts/Titania", glm::vec2(-0.95, -0.60), 70.0);
	pText hint1 = ContextGLFW::mGraphicsEngine->storeText("Use 'T' to resume/stop time", "fonts/Titania", glm::vec2(-0.95, -0.70), 70.0);
	pText hint6 = ContextGLFW::mGraphicsEngine->storeText("Use 'D' for toggle debugger On/Off", "fonts/Titania", glm::vec2(-0.95, -0.80), 70.0);
}