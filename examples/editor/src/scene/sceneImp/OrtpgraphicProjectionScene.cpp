...
void OrtpgraphicProjectionScene::init() {
	...
	mEngine->getCurrentScene()->setCamera(
	   make_unique<CameraOrthographic>(glm::vec3(0.0, 0.0,
	                                   6.0),
	                                   glm::quat(),
	                                   -10.0f,
	                                   10.0f,
	...
	/* Lights */
	mEngine->storeLightSpot(glm::vec3 (0.0, 1.0, 0.0),
	      glm::quat(),
	...
	mEngine->storeLightSpot(glm::vec3 (1.0, 1.0, 1.0),
	      glm::quat(),
	      glm::vec4 (1.0, 0.0, 0.0, 0.0),
	      lightSource_3.get());

	/* Programs */
	flc::Program* program = ProgramLoader(
	                            mEngine.get()).getDefault();

	lightSource_1->attach(make_unique<Model>(mEngine.get(),
	                      program,
	                      "meshes/sphere.obj", "0_255_0.color"));
	lightSource_2->attach(make_unique<Model>(mEngine.get(),
	                      program,
	                      "meshes/sphere.obj", "0_0_255.color"));
	lightSource_3->attach(make_unique<Model>(mEngine.get(),
	                      program,
	                      "meshes/sphere.obj", "255_0_0.color"));

	lightSource_1->attachHierarchyCallback(make_unique<TimedMoveCallback>(
	      lightSource_1.get(), glm::vec3(0.0, 0.0, 500.0), 2000.0));
	lightSource_1->scaleTo(0.02);
	lightSource_1->moveBy(glm::vec3(0.0, 0.0, 0.0));

	lightSource_2->attachHierarchyCallback(make_unique<TimedMoveCallback>(
	      lightSource_2.get(), glm::vec3(0.0, 0.0, 500.0), 2000.0));
	lightSource_2->scaleTo(0.02);
	lightSource_2->moveBy(glm::vec3(3.0, 0.0, 0.0));

	lightSource_3->attachHierarchyCallback(make_unique<TimedMoveCallback>(
	      lightSource_3.get(), glm::vec3(0.0, 0.0, 500.0), 2000.0));
	lightSource_3->scaleTo(0.02);
	lightSource_3->moveBy(glm::vec3(-3.0, 0.0, 0.0));

	mEngine->getCurrentScene()->attach(std::move(
	         lightSource_1));
	mEngine->getCurrentScene()->attach(std::move(
	         lightSource_2));
	mEngine->getCurrentScene()->attach(std::move(
	...
	mEngine->registerCallback(make_unique<MoveCameraCallback>(
	         mEngine.get(), eEventType::eKey, 0.1));
//	mEngine->registerCallback(make_unique<MoveCameraCallback>(
...
void OrtpgraphicProjectionScene::perform() {
	...
	puModel wall = make_unique<Model>(mEngine.get(), program,
	                                  "meshes/floor.obj");
	puModel shadowCastingBall1 = make_unique<Model>
	                             (mEngine.get(),
	                              program, "meshes/sphere.obj", "64_128_255.checkboard");
	puModel shadowCastingBall2 = make_unique<Model>
	                             (mEngine.get(),
	                              program, "meshes/sphere.obj", "64_128_255.checkboard");
	puModel shadowCastingBall3 = make_unique<Model>
	                             (mEngine.get(),
	                              program, "meshes/sphere.obj", "64_128_255.checkboard");
	puModel shadowCastingBall4 = make_unique<Model>
	                             (mEngine.get(),
	                              program, "meshes/sphere.obj", "64_128_255.checkboard");
	puModel shadowCastingBall5 = make_unique<Model>
	                             (mEngine.get(),
	                              program, "meshes/sphere.obj", "64_128_255.checkboard");
	...
	shadowCastingBall5->attachHierarchyCallback(make_unique<TimedMoveCallback>(
	         shadowCastingBall5.get(),
	...
	pText hint0 =
			mEngine->storeText("Fillwave example shadowing",
			                   "fonts/Titania",
