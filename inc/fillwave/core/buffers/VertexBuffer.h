...
public:
   ...
   VertexBuffer(std::vector<T>& vertices, GLuint dataStoreModification = GL_STATIC_DRAW)
    :Buffer(GL_ARRAY_BUFFER, dataStoreModification) {
   ...
   VertexBuffer(models::Shape<T>& shape, GLuint dataStoreModification = GL_STATIC_DRAW)
   :Buffer(GL_ARRAY_BUFFER, dataStoreModification) {
   ...
   void getAttributes(GLint programHandle) {
      ...
      for (int i = 0; i < howMany; ++i) {
         ...
         GLenum type = GL_ZERO;
         char name[200];
         glGetActiveAttrib(programHandle, GLuint(i), sizeof(name) - 1, &name_len,
               &num, &type, name);
         name[name_len] = 0;
