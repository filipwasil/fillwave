...
public:
   Entity(glm::mat4 model = glm::mat4(1.0),
...
private:
   ...
   void detachChildren();
   void handleEvent(std::vector<actions::ItemCallback*>& callbacks, actions::EventType* event);
   void eraseFinishedCallbacks(std::vector<actions::ItemCallback*>& callbacks);
   void detachCallback(std::vector<actions::ItemCallback*>& callbacks, actions::ItemCallback* callback);
};
} /* models */
